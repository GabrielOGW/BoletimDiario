...
export default async function SomPage({
  ...
  const equipeSom = membros.filter((membro) => membro.department === 'SOUND');
  const podeAnotar =
    hasActiveDepartment(membership.departments) &&
    membership.departments.includes('SOUND');

  return (
    <>
      <AppHeader
        title="Boletim de Som"
            ...
            <>
              {!podeAnotar ? (
                <p className="rounded-xl border border-brand/30 bg-brand-soft px-3.5 py-3 text-sm leading-relaxed text-zinc-200">
                  Você não está cadastrado no departamento de Som nesta produção, então o
                  boletim abre em modo de leitura. Um administrador pode ajustar isso na
                  tela de equipe.
                </p>
              ) : null}
              ...
              <SectionCard
                ...
                collapsible
                defaultOpen={false}
                summary={`${equipeSom.length} pessoa(s)`}
                action={
                  <Link
              ...
              >
                {equipeSom.length === 0 ? (
                  <p className="text-sm text-zinc-500">
                    Ninguém cadastrado no departamento de Som ainda.
                  </p>
                ) : (
                  <ul className="flex flex-col gap-1.5 text-sm">
                    {equipeSom.map((membro) => (
